...
describe 'collectd::plugin::apache::instance', :type => :define do
  ...
  context 'default params' do
    ...
    it { should contain_file(filename).with_content(/URL "http:\/\/localhost\/mod_status?auto"/) }
  ...
  context 'all params set' do
    ...
    it { should contain_file(filename).with_content(/URL "http:\/\/bar.example.com\/server-status?auto"/) }
    ...
    it { should contain_file(filename).with_content(/CACert "\/etc\/ssl\/certs\/ssl-cert-snakeoil.pem"/) }
  end
end
